...
{
	const HEAT_LAYER	= 100;	// sprite layer index used by the barrel heat layer

	const HEAT_MAX			= 50.0;	// maximum heat value (can't fire when you reach this)
	const HEAT_GAIN			= 0.5;	// how much heat is gained per tic when firing
	...
	override void Tick()
	{
		...
		if (owner && owner.player && owner.player.readyweapon == self)
		{
			...
			shouldGain = psp != null && Actor.InStateSequence(psp.curstate, self.GetAtkState(false));
